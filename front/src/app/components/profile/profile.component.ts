...
@Component({
  ...
  styleUrls: ['./profile.component.css']  // Fixed typo here from styleUrl to styleUrls
})
export class ProfileComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      ...
      this.loadProfile();  // Reload profile when route parameter changes
  ...
  editUser() {
    if (this.userForm.valid) {
      ...
      this.profileService.editProfile(formData).subscribe({
        next: resp => {
          if (resp) {
            window.location.reload();  // You may want to avoid this and just reload data instead
          }
        },
        error: err => {
          this.messageService.add({ severity: 'error', summary: 'Error', detail: err.error.err });
        }
      });
    }
  }
  ...
  areEqual(): boolean {
    const formValue = this.userForm.value;
    return formValue.password === formValue.repeatPassword;
  }
